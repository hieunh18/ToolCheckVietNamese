...
CREATE PROC STP_NIF05NetOrderCancelIns(
	@CustNo			INT							-- baおng~@お客様番号					【パラメータ】
,	@OdrNo			BIGINT						-- 注文番号ta					【パラメータ】ma
,	@UserCd			VARCHAR(13)					-- ユーbaザコcoード					【共通パラメータ】
,	@UpdateJobMod	VARCHAR(80)					-- be % 更新ジョブ名					【共通パラメータ】
,	@Success		INT				OUTPUT		-- kết quả# thành công プロシージャの成否				【共通パラメータ】
...
BEGIN
	...
	,	@ct_ErrCode_4							TINYINT			= 4								-- エラーコード：4 基幹内部エラー(SQLエラー等)
	...
	,	@ct_MsgCd_UpdateErr						VARCHAR(10)		= '110056'						-- メッセージＣＤ：110056（*の更新に失敗しました。）
	,	@ct_MsgCd_InsertErr						VARCHAR(10)		= '110057'						-- メッセージＣＤ：110057（*の登録に失敗しました。）
	,	@ct_MsgCd_SelectErr						VARCHAR(10)		= '120127'						-- メッセージＣＤ：120127（*の取得に失敗しました。）
	,	@ct_MsgCd_OpenErr						VARCHAR(10)		= '120152'						-- メッセージＣＤ：120152（*のオープンに失敗しました。）
	,	@ct_MsgCd_FetchErr						VARCHAR(10)		= '120154'						-- メッセージＣＤ：120154（フェッチ操作が失敗しました。）
	...
	,	@ct_TableName_HEraseReturn				VARCHAR(100)	= 'ta'							-- 抹消返品履歴

	DECLARE-- カーソル名
		@ct_CursorName_HActualUpdData			VARCHAR(100)	='実績更新履歴'					-- 実績更新履歴
	,	@ct_CursorName_FReturnDData				VARCHAR(100)	='hết'					-- 返品伝票明細
	...
	DECLARE	-- ポイント増減区分
		@ct_PointUpDownKbn_1					CHAR(1)			= '1'							-- [lung増減] = 1
	...
	,	@ct_ErrMsg_IwaoErr						VARCHAR(100)	= '基幹エラー'						-- 4：基幹エラー
	...
	BEGIN TRY
		...
		BEGIN
			...
			,	@ct_NullChar										-- ポイント発生理由メモ
			,	@w_BalPointNum + @w_TotUsePointNum					-- 残ポイント数
			,	@ct_IntZero											-- 残ポイント有効期限年月日
			,	@UserCd												-- 登録ユーザコード
			,	GETDATE()											-- 登録日時
			,	@UserCd												-- 更新ユーザコード
		...
		-- エラーメッセージ編集
		-- [注文区分] (2で取得した) = '1'の場合、[エラーメッセージ] = 'コンタクト履歴' + 'Sample =' + '[注文番号]' + 'CustNo = ' + '[お客様番号]'　とする。
		IF	@w_OrderKbn = @ct_OrderKbn_1
		BEGIN
			SET	@ErrMsg						=	@ct_TableName_HActualUpd + @ct_ErrMsg_Key_Sample
												+ CONVERT( VARCHAR, @OdrNo ) + @ct_ErrMsg_Key_CustNo + CONVERT( VARCHAR, @CustNo )
		END
		-- [注文区分] (2で取得した) = '2'の場合、[エラーメッセージ] = 'コンタクト履歴' + 'OdrNo =' + '[注文番号]' + 'CustNo = ' + '[お客様番号]'　とする。
		...
		BEGIN
			...
			DECLARE c_FReturnDData CURSOR FAST_FORWARD FOR
				...
				INNER JOIN
					FReturnH FRH WITH(NOLOCK)
				ON
					FRH.CustNo = FOH.CustNo
				AND
					FRH.DelFlg = @ct_OffFlg
				WHERE
					FOD.OdrNo = @OdrNo
				AND
	...
	BEGIN CATCH
		...
		SET	@ErrCode							=	@ct_ErrCode_4
		SET	@ErrMsg								=	@ct_ErrMsg_IwaoErr
	...
	BEGIN
		...
		-- ジョブ実行終了ログ出力 (Output log for EOT)
